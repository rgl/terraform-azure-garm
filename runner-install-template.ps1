...
function Install-Runner() {
	...
	try {
		...
		New-LocalUser -Name "garmrunner" -Password $secPasswd -PasswordNeverExpires -UserMayNotChangePassword
		$pscreds = New-Object System.Management.Automation.PSCredential (".\garmrunner", $secPasswd)
		$adminGrpName = (Get-CimInstance win32_group -Filter 'SID = "S-1-5-32-544"').Name
		if (!$adminGrpName) {
			Throw "Could not find administrators group name"
		}
		Add-LocalGroupMember -Group $adminGrpName -Member garmrunner
		$ntAcct = New-Object System.Security.Principal.NTAccount("garmrunner")
		...
		$runnerACL.SetAccessRule((New-Object System.Security.AccessControl.FileSystemAccessRule(
		    "garmrunner", "FullControl", "ContainerInherit,ObjectInherit", "None", "Allow"
		)))
